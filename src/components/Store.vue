...
<template>
  ...
  >
    ...
    <p v-if="show" class="text-yellow-300 text-center text-xs sm:text-base">
    ...
    <main v-if="show" class="px-4 w-full py-2">
      ...
      >
        <div
          class="bg-[#0f1524] w-full h-48 text-white border border-[#ffffff3b] border-dashed rounded-lg flex justify-center items-center font-bold text-2xl"
          :key="store.count"
        >
          {{ store.count }}
        </div>
    ...
    <main v-if="show" class="px-4 w-full">
      ...
      <input
        v-model="Name"
        type="text"
        class="w-full bg-gray-800 outline-none border border-[#ffffff59] border-dashed text-white p-2 rounded-md h-10"
      />
    </main>
    <div v-if="show" class="my-5">
